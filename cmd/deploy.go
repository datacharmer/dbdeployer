...
func init() {
	...
	deployCmd.PersistentFlags().Bool("enable-mysqlx", false, "Enables MySQLX plugin (5.7.12+)")
	...
	deployCmd.PersistentFlags().Bool("enable-general-log", false, "Enables general log for the sandbox (MySQL 5.1+)")
	deployCmd.PersistentFlags().Bool("init-general-log", false, "uses general log during initialization (MySQL 5.1+)")
