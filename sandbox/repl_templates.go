...
{{.SandboxDir}}/{{.MasterLabel}}/use -h {{.MasterIp}} -u {{.RplUser}} -p{{.RplPassword}} -e 'set @a=1'
...
echo 'CHANGE MASTER TO  master_host="{{.MasterIp}}",  master_port={{.MasterPort}},  master_user="{{.RplUser}}",  master_password="{{.RplPassword}}" {{.ChangeMasterExtra}}' | {{.SandboxDir}}/{{.NodeLabel}}{{.Node}}/use -u root
{{.SandboxDir}}/{{.NodeLabel}}{{.Node}}/use -u root -e 'START SLAVE'
...
then
	{{.SandboxDir}}/initialize_{{.SlaveLabel}}s
...
echo "# {{.MasterLabel}}  " 
echo "$@" | {{.SandboxDir}}/{{.MasterLabel}}/use  
...
{{.SandboxDir}}/{{.MasterLabel}}/send_kill "$@"
`
...
mstatus=$({{.SandboxDir}}/{{.MasterLabel}}/status)
if [ -f {{.SandboxDir}}/{{.MasterLabel}}/data/mysql_sandbox{{.MasterPort}}.pid ]
then
	mport=$({{.SandboxDir}}/{{.MasterLabel}}/use -BN -e "show variables like 'port'")
fi
...
{{.SandboxDir}}/{{.MasterLabel}}/test_sb "$@"
exit_code=$?
...
# Generated by dbdeployer {{.AppVersion}} using {{.TemplateName}} on {{.DateTime}}
echo "{{.MasterLabel}}"
port=$({{.SandboxDir}}/{{.MasterLabel}}/use -BN -e "show variables like 'port'")
server_id=$({{.SandboxDir}}/{{.MasterLabel}}/use -BN -e "show variables like 'server_id'")
echo "$port - $server_id"
{{.SandboxDir}}/{{.MasterLabel}}/use -e 'show master status\G' | grep "File\|Position\|Executed"
{{ range .Slaves }}
echo "{{.SlaveLabel}}{{.Node}}"
port=$({{.SandboxDir}}/{{.NodeLabel}}{{.Node}}/use -BN -e "show variables like 'port'")
server_id=$({{.SandboxDir}}/{{.NodeLabel}}{{.Node}}/use -BN -e "show variables like 'server_id'")
echo "$port - $server_id"
{{.SandboxDir}}/{{.NodeLabel}}{{.Node}}/use -e 'show slave status\G' | grep "\(Running:\|Master_Log_Pos\|\<Master_Log_File\|Retrieved\|Executed\)"
{{end}}
`
