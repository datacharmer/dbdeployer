...
echo 'CHANGE MASTER TO  master_host="{{.MasterIp}}",  master_port={{.MasterPort}},  master_user="{{.RplUser}}",  master_password="{{.RplPassword}}" ' | {{.SandboxDir}}/{{.NodeLabel}}{{.Node}}/use -u root
...
# Generated by dbdeployer {{.AppVersion}} using {{.TemplateName}} on {{.DateTime}}
echo "{{.MasterLabel}}"
{{.SandboxDir}}/{{.MasterLabel}}/use -BN -e "select CONCAT('port: ', @@port) AS port"
{{.SandboxDir}}/{{.MasterLabel}}/use -e 'show master status\G' | grep "File\|Position\|Executed"
{{ range .Slaves }}
echo "{{.SlaveLabel}}{{.Node}}"
{{.SandboxDir}}/{{.NodeLabel}}{{.Node}}/use -BN -e "select CONCAT('port: ', @@port) AS port"
