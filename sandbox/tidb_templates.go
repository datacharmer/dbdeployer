...
[ -z "$START_SLEEP_SECONDS" ] && START_SLEEP_SECONDS=1
...
then
  ...
  then
    ...
    echo "Server is not running!  Pid-file is stale."
    exit 1
  fi
else
...
then
  ...
  then
    ...
    echo "Server already shutdown! Removing stale pid-file."
    rm -f $PIDFILE
  fi
else
  echo "Server is not running!"
...
# Currently stop does the same thing in TiDB
$SBDIR/stop
